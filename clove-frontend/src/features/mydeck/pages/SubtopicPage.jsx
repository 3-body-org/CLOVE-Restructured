    ...
    >
      ...
      <div ref={svgContainerRef} style={{ position: 'relative', width: '100%', minHeight: '600px' }}>
        ...
        <SubtopicLayout
           ...
           subtopics={(() => {
             const subtopicsObj = Object.fromEntries((subtopics || []).map(subtopic => [subtopic.key, subtopic]));
             // If no backend data yet, create placeholder subtopics
             if (!subtopics || subtopics.length === 0) {
               return Object.fromEntries((nodeOrder || []).map(key => [key, {
                 key,
