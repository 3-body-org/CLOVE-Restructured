...
async def submit_single_answer(
    ...
    # Get or create post_assessment record
    existing = await get_existing_post_assessment(db, user_topic_id)
...
async def _create_new_assessment(
    ...
    new_post_assessment = PostAssessment(
    ...
    db.add(new_post_assessment)
    await db.commit()
    await db.refresh(new_post_assessment)
