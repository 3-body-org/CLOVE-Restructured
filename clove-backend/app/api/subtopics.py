...
async def read_subtopic(subtopic_id: int, db: AsyncSession = Depends(get_db)):
...
async def update_subtopic(subtopic_id: int, subtopic_in: SubtopicUpdate, db: AsyncSession = Depends(get_db)):
    sub_obj = await get_by_id(db, subtopic_id=subtopic_id)
    if not sub_obj:
        raise HTTPException(status_code=404, detail="Subtopic not found")
